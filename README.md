...
- **First-time visitors** get a one-time offer from Solis ("Want a quick tour?").
  Accepting walks through the main sections with a spotlight effect; "Maybe
  later" doesn't ask again automatically, but anyone can trigger it anytime by
  asking Solis for a "tour". Completion is remembered via `localStorage`
  (`tourCompleted`) — delete that key in DevTools to make it offer again.
- **The "What's New" popup** shows once per version. To ship a real update:
  bump `CHANGELOG_VERSION` in `js/app.js` and update `CHANGELOG_DATA` with what
  actually changed. Visitors who already dismissed the current version won't see
  it again until you bump the version string.

## Social media links

Open `config.js` and fill in `SOCIAL_LINKS` with your club's real profile URLs.
Any platform left as `""` just doesn't show an icon in the footer — no need to
delete anything, only fill in the ones you actually use.

## Event requests from other clubs

There's a public form (no account needed) at the bottom of the homepage — reachable
via the "For Clubs" nav link — where officers from other clubs can ask to have their
event listed on your Events page. Submissions show up under **Event requests** in
the admin dashboard, where an admin can:
- **Approve & post** — creates a real row in `events` immediately and marks the
  request approved.
- **Decline** — marks it declined; it stays visible in the admin table for the
  record, with a **Remove** button if you want it gone entirely.

This ships in `schema.sql` already. If you're adding it to an existing project,
run `supabase/migrations/003_add_event_requests.sql` instead.
